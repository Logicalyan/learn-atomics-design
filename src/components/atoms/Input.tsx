...
export const Input: React.FC<InputProps> = ({
  ...
  const baseClasses =
    'border rounded focus:outline-none focus:ring-2 transition-all w-full';
    ...
    <div className="flex flex-col gap-1 w-full">
      ...
      <div className="relative w-full">
        {icon && iconPosition === 'left' && (
          <span
            className={`absolute left-3 top-1/2 -translate-y-1/2 ${iconColorClasses[variant]}`}
          >
            {icon}
          </span>
        )}
        ...
        {icon && iconPosition === 'right' && (
          <span
            className={`absolute right-3 top-1/2 -translate-y-1/2 ${iconColorClasses[variant]}`}
          >
            {icon}
          </span>
        )}
